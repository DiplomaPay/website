<?php


?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Landpage</title>
</head>
<body>
    <header>
        <img src="../items/img/IconBranco.svg" alt="Icone da DiplomaPay">
        <div>
            <a href="#">Produtos</a>
            <a href="#">Preços</a>
            <a href="#">Simulador</a>
            <a href="#">Quem somos</a>
            <button>Entrar</button>
        </div>
    </header>

    <section class="start">
        <div>
            <h1>A conta online para grupos de formaturas</h1>
            <p>O único serviço digital que te auxilia a organizar uma formatura incrível de forma segura e econômica.</p>
            <button>Abrir conta grátis</button>
        </div>
    

        <div>
            <div>
                <div>
                    <p>Mais de</p>
                    <strong>R$ 200 MIL</strong>
                    <p>Arrecadados por turma</p>
                </div>
                <strong>+400</strong>
                <p>Formandos registrados</p>
                <strong>+20</strong>
                <p>Turmas que contam conosco</p>
                <button>Faça uma simulação</button>
            </div>
            <div>
                <div>
                    <h2>Conheça nossos planos</h2>
                    <a href="Ver agora >"></a>
                </div>
                <div>
                    <h2>Para empresas</h2>
                    <p>Focado em conectar formandos e empresas </p>
                    <a href="Ver agora >"></a>
                </div>
            </div>
        </div>
    </section>

    <section class="info">
        <div>
            <p>O APP CHEGOU NA HORA CERTA</p>
            <h2>O sistema perfeito para organizar formaturas</h2>
        </div>

        <div>
            <div>
                <img src="" alt="">
                <h3>Aqui seu dinheiro rende mais</h3>
                <p>Com a nossa plataforma, seu dinheiro trabalha para você, rendendo 100% do CDI. Quanto mais você investe, mais você ganha!</p>
            </div>
            <div>
                <img src="" alt="">
                <h3>Faça do seu jeito</h3>
                <p>Arrecade com nossos diversos métodos, tenha total transparência do que acontece na conta de sua turma.</p>
            </div>
            <div>
                <img src="" alt="">
                <h3>Segurança de ponta</h3>
                <p>Proteger suas informações é nossa prioridade máxima, garantindo uma experiência segura e confiável para você.</p>
            </div>
        </div>
    </section>

    <section class="planos">
        <h2>Nossos planos para sua turma</h2>

        <div>
            <div>
                <div>
                    <img src="" alt="estrela">
                    <p>Premium</p>
                    <p>R$0.99<span>/mês</span></p>
                    <p>por aluno</p>

                    <ul>
                        <li>
                            Rendimento 100% CDI
                        </li>
                        <li>
                            Rifas
                        </li>
                        <li>
                            Lojinhas
                        </li>
                        <li>
                            Votações
                        </li>
                        <li>
                            Comunicações
                        </li>
                        <li>
                            Envio de vendas por e-mail
                        </li>
                        <li>
                            Contrato digital
                        </li>
                        <li>
                            Prioridade no suporte
                        </li>
                        <li>
                            Relatórios completos
                        </li>
                        <li>
                            MarketPlace
                        </li>
                        <li>
                            1 Reunião mensal (Adicional estrategista free)
                        </li>
                    </ul>
                    <button>Começar agora</button>
                </div>
                <div>
                    <p>Taxas premium</p>
                    <p>Administrativo: 2% + R$0,20</p>
                    <p>6% por Pix</p>
                    <p>10% Cartão de crédito</p>
                    <p>R$6 por Boleto</p>
                </div>

            </div>
            <div>
            <div>
                    <p>Normal</p>
                    <p>Sem mensalidade</p>
                    <p>Não pague pelo serviço</p>

                    <ul>
                        <li>
                            Rifas
                        </li>
                        <li>
                            Lojinha
                        </li>
                        <li>
                            Comunicados
                        </li>
                        <li>
                           Contrato digital
                        </li>
                        <li>
                            Relatórios simples
                        </li>
                    </ul>
                    <button>Criar agora</button>
                </div>
                <div>
                    <p>Taxas normais</p>
                    <p>Administrativo: 3% + R$0,40</p>
                    <p>9% por Pix</p>
                    <p>14% Cartão de crédito</p>
                    <p>R$8 por Boleto</p>
                </div>
            </div>
        </div>
    </section>

    <footer>
        <img src="../items/img/IconBranco.svg" alt="icone Diplomapay">
    </footer>
</body>
</html>